...
def validate_and_quarantine(data: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    if tuple(data.columns) != EXPECTED_COLUMNS:
        raise ValueError(f"Schema mismatch. Expected columns: {list(EXPECTED_COLUMNS)}")
    ...
    invalid_label = checked[TARGET_COLUMN].isna() | ~checked[TARGET_COLUMN].isin(TARGET_CLASSES)
    ...
    for species, group in checked.groupby(TARGET_COLUMN):
        valid_group = group[list(FEATURE_COLUMNS)].dropna()
        if species not in TARGET_CLASSES or len(valid_group) < 10:
            continue
        median = valid_group.median()
        mad = (valid_group - median).abs().median().replace(0, 1e-6)
        robust_z = 0.6745 * (group[list(FEATURE_COLUMNS)] - median).abs() / mad
